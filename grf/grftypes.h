...
typedef struct {
    ...
    uint32_t
        line; /**<  Line number in the soure code where the error took place */
    const char
        *file; /**<  Filename of source code where the error took place. */
 ...
 *
 * @note GRAVITY's GrfFile struct is 0x114 bytes, in the same order as this
 * structure.
 */
typedef struct _GrfFile {
    /** Size in file.
     * If using any form of DES encryption, this must be a multiple of 8 (which
     * is the block size).
     */
 ...
 */
#define GRFFILE_IS_DIR(f)                                                      \
    (((f).flags & GRFFILE_FLAG_FILE) == 0 ||                                   \
...
typedef struct {
    ...
    int64_t len;    /**<  Size of the GRF file */
    ...
    /* Private fields */
    uint8_t
        allowCrypt; /**<  Internal use only - Can files be encrypted or not? */
